...
def run_matching(cur, bank_tx_id, company_id, amount, payment_purpose, matched_by=None):
    ...
    # Шаг 2а: найти счёт по номеру из назначения платежа
    if invoice_number:
        cur.execute(
    ...
    # Шаг 2б: поиск среди открытых счетов
    if not invoice_row:
        cur.execute(
            """SELECT id, invoice_number, total_vat, status
               FROM invoice
        ...
        if len(open_invoices) == 0:
            # Нет открытых счетов — зачисляем на баланс
        ...
        if len(exact) == 1:
            ...
            # Несколько совпадений или несколько открытых — нужно ручное распределение
            notify_users_by_role(
                cur, "manager", "payment_needs_distribution",
            ...
            # Единственный открытый счёт — привязываем невзирая на сумму
    ...
    # Счёт уже оплачен — остаток на баланс
    if inv_status == "paid":
        _add_to_balance(cur, company_id, amount, bank_tx_id, payment_purpose)
        return "overpayment"

    # Шаг 3: сравниваем сумму
...
def _pay_invoice(cur, inv_id, inv_number, company_id, amount,
    ...
    # Привязываем транзакцию к счёту + фиксируем кто сопоставил
    cur.execute(
        """UPDATE bank_transaction
           SET matched_invoice_id = %s, matched_at = %s,
               matched_by = %s
           WHERE id = %s""",
        (inv_id, now, matched_by, bank_tx_id),
    )

    # Помечаем заказы оплаченными
    cur.execute(
    ...
    if overpayment > Decimal("0"):
        ...
        notify_company_users(
            ...
            f"Счёт {inv_number} оплачен. "
            f"Переплата {overpayment} ₽ зачислена на ваш баланс.",
            "invoice", inv_id,
        )
        notify_users_by_role(
    ...
    # Снять системную блокировку при оплате
    cur.execute(
        """UPDATE company
    ...
    # Уведомить логиста — можно отгружать
    cur.execute(
...
def _add_to_balance(cur, company_id, amount, bank_tx_id, payment_purpose):
    """Зачислить платёж на баланс клиента (нет открытых счетов / переплата)."""
    cur.execute("SELECT balance FROM company WHERE id = %s", (company_id,))
    row = cur.fetchone()
    if not row:
        return
