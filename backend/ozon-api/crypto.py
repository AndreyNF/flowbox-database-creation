...
def encrypt_key(plaintext: str) -> str:
    """Шифрует строку → возвращает 'enc:v1:<base64>'."""
    if not plaintext:
        return plaintext
...
def decrypt_key(ciphertext: str) -> str:
    """Расшифровывает 'enc:v1:<base64>' → исходную строку.
    Если значение без префикса — возвращает как есть (legacy)."""
    if not ciphertext:
        return ciphertext
    if not ciphertext.startswith(PREFIX):
        return ciphertext  # legacy: открытый текст
    key    = _get_key()
...
def mask_key(raw: str) -> str:
    """Маска для UI: abcd...wxyz. Работает с зашифрованным и открытым текстом."""
    if not raw:
        return ""
    if raw.startswith(PREFIX):
        # Показываем только хвост base64 для идентификации
        b64 = raw[len(PREFIX):]
